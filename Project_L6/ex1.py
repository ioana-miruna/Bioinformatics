...
def read_fasta(file_path):
    if os.path.isdir(file_path):
        fasta_files = [f for f in os.listdir(file_path) if f.lower().endswith(('.fasta', '.fa'))]
        if not fasta_files:
            raise FileNotFoundError(f"No FASTA file found in directory: {file_path}")
        file_path = os.path.join(file_path, fasta_files[0])
        print(f"Detected FASTA file: {file_path}")
...
def main():
    ...
    sequence = read_fasta(r'/Users/ioanabadica/Documents/UNI/YEAR 4/BioInformatics/LAB6')
